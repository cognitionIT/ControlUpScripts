<#
.SYNOPSIS
    Get WVD 2020 Spring Release Host Pool information, sorted by Name.
.DESCRIPTION
    Get WVD 2020 Spring Release Host Pool information, using the new Az.Desktopvirtualization PowerShell Module and WVD ARM Architecture (2020 Spring Release).
.EXAMPLE
    Get-WVDHostPool
.EXAMPLE
...
.NOTES
    ...
    Updated:        2020-06-28
                    Replaced the HostPool Name parameter with the HostPool ID parameter, that will be provided by the Description field from the CU Console
    Updated:        2020-07-05
...
If (Invoke-NETFrameworkCheck)
{
    If ($azSPCredentials = Get-AzSPStoredCredentials)
    {
        ...
        {
            #Retrieve the WVD Host Pool information for this Subscription
            [array]$psobjectCollection = Get-WVDHostPool -SubscriptionID $($azSubscription.Id) -HostPoolName $wvdHostPoolName
        }
        catch
        {
            Write-Error ("A [" + $_.Exception.GetType().FullName + "] ERROR occurred. " + $_.Exception.Message)
        ...
        # Return the Host Pool information (limited details with Select)
        Write-Host "WVD 2020 Spring Release Host Pools: " -ForegroundColor Yellow
        if ($psobjectCollection.Count -gt 0)
        {
            $psobjectCollection | Sort-Object Name | Select Name, ResourceGroup, Location, HostPoolType, MaxSessionLimit, LoadBalancerType, @{Name='ApplicationGroups'; Expression={$_.ApplicationGroupsCount}}, FriendlyName, Description | Format-Table -AutoSize
        }
        else 
        {
            Write-Warning "No Host Pool(s) information could be retrieved"
        }
    }
    else 
    {
