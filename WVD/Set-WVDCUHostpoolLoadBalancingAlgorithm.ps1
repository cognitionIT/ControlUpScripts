<#
.SYNOPSIS
    Set WVD 2020 Spring Release Hostpool Max Session Limit.
.DESCRIPTION
    Set WVD 2020 Spring Release Hostpool Max Session Limit, using the new Az.Desktopvirtualization PowerShell Module and WVD ARM Architecture (2020 Spring Release).
.EXAMPLE
    Set-WVDHostpoolMaxSessionLimit -Name <HostpoolName>
.CONTEXT
    Windows Virtual Desktops
.MODIFICATION_HISTORY
    Esther Barthel, MSc - 25/05/20 - Original code
    Esther Barthel, MSc - 25/05/20 - Changed the script to support WVD 2020 Spring Release (ARM Architecture update)
.LINK
...
.NOTES
    ...
    Updated:        2020-06-28
                    Changed the script to use the ControlUp Description (ID) as input parameter
    Updated:        2020-07-10
...
(
    ...
    [Parameter(
        Position=1, 
        Mandatory=$true, 
        HelpMessage='specify the Max Session Limit'
    )]
...
If (Invoke-NETFrameworkCheck)
{
    If ($azSPCredentials = Get-AzSPStoredCredentials)
    {
        ...
        try 
        {
            #Retrieve the WVD Host Pool information for this Subscription and name
            [array]$hostPools = Get-WVDHostPool -SubscriptionID $($azSubscription.Id) -HostPoolName $wvdHostPoolName
        }
        catch
        {
            Write-Error ("A [" + $_.Exception.GetType().FullName + "] ERROR occurred. " + $_.Exception.Message)
        ...
        If ($hostPools.Count -gt 0)
        {
            ...
            {
                Update-WVDHostpool -HostpoolName $($hostPool.Name) -ResourceGroupName $hostPool.Id.Split("/")[4] -LoadBalancingAlgorithm $LoadBalancingAlgorithm
                Write-Host ("Hostpool '$($hostPool.Name)' load balancing algorithm updated to: ") -ForegroundColor Yellow -NoNewline
                Write-Host $($LoadBalancingAlgorithm) -ForegroundColor Cyan
            }
        }
    }
    else 
