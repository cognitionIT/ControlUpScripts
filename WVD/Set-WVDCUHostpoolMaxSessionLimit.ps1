...
.EXAMPLE
    Set-WVDHostpoolMaxSessionLimit -SessionHostName <SessionHostName>
.CONTEXT
...
If (Invoke-NETFrameworkCheck)
{
    If ($azSPCredentials = Get-AzSPStoredCredentials)
    {
        ...
        If ($hostPools.Count -gt 0)
        {
            ...
            {
                Update-WVDHostpool -HostpoolName $($hostPool.Name) -ResourceGroupName $hostPool.Id.Split("/")[4] -MaxSessionLimit $MaxSessionLimit
                Write-Host ("Hostpool '$($hostPool.Name)' max session limit updated to: ") -ForegroundColor Yellow -NoNewline
                Write-Host $($MaxSessionLimit) -ForegroundColor Cyan
            }
        }
    }
    else 
