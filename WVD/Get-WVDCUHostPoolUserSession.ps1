<#
.SYNOPSIS
    Get WVD 2020 Spring Release User Session information, sorted by HostPoolName and SessionHostName.
.DESCRIPTION
    Get WVD 2020 Spring Release User Session information, using the new Az.Desktopvirtualization PowerShell Module and WVD ARM Architecture (2020 Spring Release).
.EXAMPLE
    Get-WVDUserSession 
.CONTEXT
    Windows Virtual Desktops
.MODIFICATION_HISTORY
    Esther Barthel, MSc - 12/06/20 - Original code
    Esther Barthel, MSc - 12/06/20 - Standardizing script, based on the ControlUp Scripting Standards (version 0.2)
.LINK
...
(
    ...
    [Parameter(
        Position=1, 
        Mandatory=$false, 
        HelpMessage='Enter the UPN of the user (or the username in front of the @)'
    )]
...
#region Retrieve input parameters
    # Check if the UserPrincipalName parameter is empty or not
...
If ($azSPCredentials = Get-AzSPStoredCredentials)
{
    ...
    try 
    {
        ...
        $Id = (Get-WVDSessionHostHostPool -SubscriptionID $($azSubscription.Id) -SessionHostName $SessionHostName).HostPoolId
        # Translate ID to HostPool Name
    ...
    try 
    {
        #Retrieve the WVD User Session information for this Subscription
        [array]$psobjectCollection = Get-WVDHostPoolUser -SubscriptionId $($azSubscription.Id.ToString()) -HostPoolName $wvdHostPoolName -UPN $UserPrincipalName
    }
    catch
    {
        Write-Error ("A [" + $_.Exception.GetType().FullName + "] ERROR occurred. " + $_.Exception.Message)
    ...
    # Return the User Session information (limited details with Select)
    Write-Host "WVD 2020 Spring Release Host Pool - User Sessions: " -ForegroundColor Yellow
    if ($psobjectCollection.Count -gt 0)
    {
        $psobjectCollection | Sort-Object HostPoolName, SessionHostName | Select @{Name='HostPool'; Expression={$_.HostPoolName}}, @{Name='SessionHost'; Expression={$_.SessionHostName.Split(".")[0]}}, @{Name='SessionID'; Expression={$_.UserSessionID}}, @{Name='UPN'; Expression={$_.UserPrincipalName}}, @{Name='State'; Expression={$_.SessionState}}, ApplicationType, CreateTime, @{Name='ResourceGroup'; Expression={$_.HostpoolResourceGroup}} | Format-Table -AutoSize
    }
    else 
    {
        Write-Warning "No User Session(s) information could be retrieved"
    }
}
else 
{
