<#
.SYNOPSIS
    Get WVD 2020 Spring Release Host Pool VM Image information, sorted by Name.
.DESCRIPTION
    Get WVD 2020 Spring Release Host Pool VM Image information, using the new Az.Desktopvirtualization PowerShell Module and WVD ARM Architecture (2020 Spring Release).
.EXAMPLE
    Get-WVDHostPoolVMImage
.EXAMPLE
...
If (Invoke-NETFrameworkCheck)
{
    If ($azSPCredentials = Get-AzSPStoredCredentials)
    {
        ...
        # Return the Host Pool VMTemplate information (limited details with Select)
        if ($psobjectCollection.Count -gt 0)
        {
            foreach ($item in $psobjectCollection)
            {
                Write-Host "Image information for HostPool '$($item.Name)': " -ForegroundColor Yellow
                If ($item.VMTemplate)
                {
                    ($item.VMTemplate | ConvertFrom-Json)#.vmSize.id
                    Write-Host ""
                }
                else 
                {
                    Write-Warning "No image information found"
                }
            }
        }
        else 
